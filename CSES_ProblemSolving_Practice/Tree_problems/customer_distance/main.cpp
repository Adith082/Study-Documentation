...
int find_solution(int node1, int node2) {
    ...
    if(node1 == node2) {
        return ((lv1 - node_level[node1]) + (lv2 - node_level[node2]));
    }
    for(int i = 19;i>=0;i--) {
    ...
    int res = (lv1 - node_level[dp[node1][0]]) + (lv2 - node_level[dp[node1][0]]);
    return res;
}

int main() {

    long long int i,j,t,n,q, val, x, k, y, p, qq;
